...
from kkitadmin.permission_list import perm_dic
...
def perm_check(*args,**kwargs):
    ...
    request = args[0]
    # print(request.user.is_authenticated, 'args[0]', args[0].user, '*args', args)
    # 获取用户访问的的url_name
    ...
    for permission_key,permission_val in  perm_dic.items():
        ...
        print('permission_val############',permission_val,'aaaaaa',len(permission_val))
        if len(permission_val) > 4:
            perm_hook_func = permission_val[4]
        else:
            perm_hook_func= None
        ...
        if per_url_name == current_url_name: #matches current request url
            if per_method == request.method: #matches request method
                ...
                if perm_hook_func:
                    ...
                    print('perm_hook_func(request)',perm_hook_func(request))
                print('perm_hook_matched',perm_hook_matched)
    ...
    if all(match_results):
        # 第一个值给app_name，其余的都给*per_name，这个值是个列表
...
def check_permission(func):
    def inner(*args,**kwargs):
        ...
        if not perm_check(*args,**kwargs):
            # 将访问的request对象赋值args[0]= 相当于这个里面的参数的第一个参数request，def get_filter_result(request,querysets)
            request = args[0]
            # 并且返回403没有权限
            return render(request,'kkitadmin/page_403.html')
        # 如果权限符合，那么返回 request所访问的视图函数，也就是正常执行了
